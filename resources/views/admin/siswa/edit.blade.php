<x-dashboard-layout :title="'Edit Siswa - '.$siswa->nama">
    ...
    <div class="bg-white p-6 rounded-xl shadow border border-gray-200 max-w-2xl">
        <form action="{{ route('admin.siswa.update', $siswa->id_siswa) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama" value="{{ old('nama', $siswa->nama) }}" class="w-full mt-1 px-4 py-2 border rounded-lg" required>
                @error('nama')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_orangtua" value="{{ old('nama_orangtua', $siswa->nama_orangtua) }}" class="w-full mt-1 px-4 py-2 border rounded-lg">
            </div>
            ...
            <div class="mb-4 grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $siswa->tanggal_lahir ? date('Y-m-d', strtotime($siswa->tanggal_lahir)) : '') }}" class="w-full mt-1 px-4 py-2 border rounded-lg">
                </div>
            </div>

            <div class="mb-6">
                <label class="font-medium block mb-1">Alamat</label>
                <textarea name="alamat" rows="3" class="w-full mt-1 px-4 py-2 border rounded-lg">{{ old('alamat', $siswa->alamat) }}</textarea>
            </div>
